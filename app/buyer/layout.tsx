            ...
            <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-6 md:px-12">
                    <div className="flex items-center justify-between h-20">
                        {/* Logo */}
                        <Link href="/buyer/products" className="flex items-center gap-3 hover:opacity-90 transition-opacity">
                        ...
                        <div className="hidden md:flex items-center space-x-8">
                            ...
                            <Link
                                href="/buyer/cart"
                                className="text-sm font-medium text-slate-500 hover:text-[#2B2640] transition-colors relative flex items-center"
                            >
                                Cart
                                {cartCount > 0 && (
                                    <span className="absolute -top-2.5 -right-3.5 bg-[#2B2640] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                                        {cartCount}
                                    </span>
                                )}
                            </Link>
                    ...
                    <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${mobileMenuOpen ? 'max-h-96 opacity-100 py-4 border-t border-slate-100' : 'max-h-0 opacity-0'}`}>
                        <div className="flex flex-col space-y-4 px-2">
                            ...
                            <Link
                                href="/buyer/cart"
                                className="text-sm font-medium text-slate-600 hover:text-[#2B2640] flex justify-between"
                                onClick={() => setMobileMenuOpen(false)}
                            >
                                <span>Cart</span>
                                {cartCount > 0 && (
                                    <span className="bg-[#2B2640] text-white text-xs font-bold px-2 py-0.5 rounded-full">
                                        {cartCount}
                                    </span>
                                )}
                            </Link>
